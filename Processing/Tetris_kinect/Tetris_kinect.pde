...
// Kinect interactivity by #interactivas interactivas17.wordpress.com
// Controls:
// left/right : movement
//         up : rotation
//   spacebar : Drop Block
...
boolean rotarON = false;
boolean derechaON = false;
boolean izquierdaON = false;
...
void draw() {
  ...
  if (game == 2) pauseText();
     ...
     PVector cadera = new PVector();
     PVector manoderecha = new PVector();
     PVector manoizq = new PVector();
     
     context.getJointPositionSkeleton(1,SimpleOpenNI.SKEL_TORSO,cadera);
     context.getJointPositionSkeleton(1,SimpleOpenNI.SKEL_RIGHT_HAND,manoderecha);
     context.getJointPositionSkeleton(1,SimpleOpenNI.SKEL_LEFT_HAND,manoizq);


     println("cadera:" + cadera.y);
     println("mano derecha x: " + manoderecha.x);
     println("mano izquierda x" + manoizq.x);
     println("mano derecha y:" + manoderecha.y);
     println("mano izquierda y:" + manoizq.y);
     println("usuarios:" + context.getUsers().length);

   if (block.size() == 4 && game == 0 && block.get(0).y > 1) {
    // movimientos para jugar, kinect tracking
    if (context.isTrackingSkeleton(1)){
        println("trackeado");    
        if (cadera.y > 200) blockFall(); // movimiento de saltar para que la pieza caiga
        
        else if (manoderecha.x > 550 && derechaON == false ) // mover pieza a la derecha con la mano der.
        {
          blockRight();  
          derechaON = true;
        }
      
       else if (manoizq.x < -300 && izquierdaON == false ) // mover pieza a la izquierda con la mano izq.
        {
          blockLeft();  
          izquierdaON = true;
        }
        ...
        else if (manoizq.y > 400 && manoderecha.y > 400 && rotarON == false ) // rotar piezas al unir manos
        {
            blockRot(); 
            rotarON = true; 
        }
        
        if (manoizq.y < 200 && manoderecha.y < 200 ) 
            rotarON = false;
          
        if (manoderecha.x < 450) 
            derechaON = false;
